  ...
  tabs.forEach(tab => {
    ...
    tab.addEventListener("click", () => {
      ...
      panels.forEach(panel => {
        const { dataset } = panel;
        const { panel: panelData } = dataset;

        panel.toggleAttribute("hidden", tabData !== panelData);
      });
    });
  });
});
...
productsInners.forEach(inner => {
  const productsSlider = inner.querySelector(".products-slider");
  const prevArrow = inner.querySelector(".slider-arrow--prev");
  const nextArrow = inner.querySelector(".slider-arrow--next");

  if (productsSlider) {
    /** @type {NodeListOf<HTMLDivElement>} */
    const productsCardTop = productsSlider.querySelectorAll(".product-card__top");

    productsCardTop.forEach(top => {
      const imagesSlider = top.querySelector(".product-images");
      const pagination = top.querySelector(".slider-pagination");

      if (imagesSlider) {
        /** @type {HTMLElement} */
        const firstImg = imagesSlider.querySelector(".ibg");

        const swiper = new products_Swiper(imagesSlider, {
          modules: [products_Pagination,],
          pagination: {
            clickable: true,
            el: pagination,
            enabled: true,
        ...
        if (firstImg) {
          const firstImgResizeObserver = new ResizeObserver(entries => {
            entries.forEach(entry => {
              const { borderBoxSize } = entry;
              const { blockSize } = borderBoxSize[0];

              inner.style.setProperty("--product-arrow-top", `${blockSize / 2}px`);
            });
          });

          firstImgResizeObserver.observe(firstImg);
        }
      }
    });

    const swiper = new products_Swiper(productsSlider, {
      modules: [products_Keyboard, products_Navigation,],
      keyboard: {
        enabled: true,
        pageUpDown: false,
      },
